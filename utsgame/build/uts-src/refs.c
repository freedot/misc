/* https://github.com/creationix/dukluv/blob/master/src/refs.c */
#define DUK_COMPILING_DUKTAPE
#include "refs.h"

/* Create a global array refs in the heap stash. */
DUK_EXTERNAL void duv_ref_setup(duk_context *ctx) {
...
DUK_EXTERNAL int duv_ref(duk_context *ctx) {
  /* stack [ value ]*/
  int ref;
  ...
  duk_remove(ctx, -2); /* stack [ value, refs ]*/

  /* ref = refs[0] */
  duk_get_prop_index(ctx, -1, 0);
  ref = duk_get_int(ctx, -1);
  duk_pop(ctx);/* stack [ value, refs ]*/
  ...
  duk_insert(ctx, -2); /* stack [ refs, value ] */

  /* refs[ref] = value */
  duk_put_prop_index(ctx, -2, ref); /* stack [ refs ] */

  /* Remove the refs array from the stack. */
  duk_pop(ctx); /* stack [ empty ] */

  return ref;
}

DUK_EXTERNAL void duv_push_ref(duk_context *ctx, int ref) {
  if (!ref) {
...
DUK_EXTERNAL void duv_unref(duk_context *ctx, int ref) {
