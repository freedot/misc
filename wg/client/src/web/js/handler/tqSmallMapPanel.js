...
SmallViewmap = Class.extern(function(){
	//SmallViewmap-unittest-start
	var m_this=null;
	var m_g=null;
	var m_caller=null;
	var m_mapdom=null;
	var m_viewblock=null;
	var m_eventdom=null;
	...
	var _setViewportPos = function(pos){
		...
		m_viewblock.style.left = m_viewport.x+'px';
		m_viewblock.style.top = m_viewport.y+'px';
	};
...
SmallMapPanel = SmallViewmap.extern(function(){
	...
	this.setTitle = function(title){
		TQ.setTextEx(m_items.title, title);
	};
	...
	this.getMapEventDom = function(){
		return m_items.smap_event;
	};
	
	var _initParams = function(selfThis, g, dom){
		m_this = selfThis;
		m_g = g;
		m_parent = dom;
	};
	
	var _createUIItems = function(){
		m_g.getGUI().initPanel(m_parent, uicfg.main.smallmap, m_items);
		TQ.setCSS(m_items.smap_viewblock_i, 'opacity', 15);
	};
	
	var _callSuperInit = function(){
		// super的实现有问题，只能在一层次的继承中调用，慎用！！！
		// 当 init(...)的参数中又调用了当前层次的方法，下面的保护措施将发生错误。
		// 目前还想不到好的方法，所以慎用！！！
		var Super = m_this.Super;
		m_this.Super = m_this.Super.Super;
		Super.init(m_g, m_items.smallmap, m_items.smap_viewblock, m_items.smap_event);
		m_this.Super = Super;
	};
