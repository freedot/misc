...
FarmBuildBlock = function(){
	//FarmBuildBlock-testunit-start
	var m_g=null;
	var m_this=null;
	var m_dom=null;
	var m_commobj=null;
	var m_item=null;
	var m_canGatherDom=null;
	var m_protectDom=null;
	...
	this.clear = function(){
		...
		m_g.unregUpdater(m_this, _onProtectStop);
	};
	
	this.getORect = function(){
		return m_commobj.getORect();
	};
	...
	this.hide = function(){
		m_commobj.hide();
		TQ.setCSS(m_canGatherDom, 'visibility', 'hidden' );
		TQ.setCSS(m_protectDom, 'visibility', 'hidden' );
		m_g.unregUpdater(m_this, _onProtectStop);
	};
	
	this.isShow = function(){
		return m_commobj.isShow();
	};
	...
	var _createCanGatherDom = function(){
		m_canGatherDom = TQ.createDom('div');
		TQ.append(m_dom, m_canGatherDom);
		TQ.setClass(m_canGatherDom, 'farmblockcangather');
	};
	
	var _createProtectDom = function(){
		m_protectDom = TQ.createDom('div');
		TQ.append(m_dom, m_protectDom);
		TQ.setClass(m_protectDom, 'farmblockprotect');
	};
	
	var _hot = function(){
		IMG.setBKImage(m_dom, IMG.makeFarmBuildImg(m_item.itemres.img, true, m_item.state));
	};
	
	var _normal = function(){
		IMG.setBKImage(m_dom, IMG.makeFarmBuildImg(m_item.itemres.img, false, m_item.state));
	};
	...
	var _setCanGatherVisible = function(){
		if ( m_item && m_item.canGather ) {
			TQ.setCSS(m_canGatherDom, 'visibility', 'visible' );
		}
		else {
			TQ.setCSS(m_canGatherDom, 'visibility', 'hidden' );
		}
	...
	var _setProtectDomVisible = function(){
		if ( _isInProtectTime() ) {
			TQ.setCSS(m_protectDom, 'visibility', 'visible' );
			var leftTime = (m_item.pStopTime - m_g.getSvrTimeS())*1000;
			m_g.regUpdater(m_this, _onProtectStop, leftTime, true);
		}
		else {
			TQ.setCSS(m_protectDom, 'visibility', 'hidden' );
			m_g.unregUpdater(m_this, _onProtectStop);
	...
	var _isInProtectTime = function(){
		return m_item 
			&& m_item.state
			&& (m_item.state == FARM_STATE.COMPLETE)
			&& m_item.pStopTime 
			&& (m_g.getSvrTimeS() < m_item.pStopTime);
	};
	
	var _onProtectStop = function(){
		TQ.setCSS(m_protectDom, 'visibility', 'hidden' );
		m_g.unregUpdater(m_this, _onProtectStop);
	};
...
FarmModel = Class.extern(function(){
	//FarmModel-testunit-start
	var m_g=null;
	var m_this=null;
	var m_blocktip=null;
	var m_curfarm={ver:-1,role:{uid:-1, citylevel:-1},blocks:[]};
	var m_observer=null;
	...
	this.init = function(g) {
		m_g = g;
		m_this = this;
		m_blocktip = FarmBlockTip.snew(m_g, this);
		FarmSender.initg(m_g);
	};
	...
	this.handleFarmSvrData = function(ndata) {
		_handleFarmData(ndata.data.farm);
	...
	this.updateBlocks = function(){
		var servertime = m_g.getSvrTimeS();
		var completeids = [];
		var statechangeids = [];
		for ( var i=0; i<m_curfarm.blocks.length; ++i ) {
			var block = m_curfarm.blocks[i];
		...
		FarmSender.sendCompleteFarmBlocks(m_curfarm.role.uid, completeids);
	...
	this.isMyFarm = function(farm){
		...
		var roleid = m_g.getImgr().getRoleRes().uid;
	...
	this.getCanGetMyRes = function(resid){
		var totalres = 0;
		var myfarm = m_g.getImgr().getMyFarm();
	...
	this.getTotalBlockCount = function(){
		return _getTotalBlockCount(m_curfarm.role.citylevel);
	};
	
	this.getNextBlockCount = function(){
		return _getNextBlockCount(m_curfarm.role.citylevel);
	};
	
	this.getBlock = function(blockid) {
		return TQ.find(m_curfarm.blocks, 'id', blockid);
	};
	...
	this.getBlockResInfo = function(block){
		if ( block.state == FARM_STATE.COMPLETE ) {
			...
			var duration = block.stoptime - block.starttime;
			var lefttime = block.stoptime - m_g.getSvrTimeS();
	...
	var _resetFarmBlocksByCityLevel = function(netfarm, farm){
		...
		_resetFarmEmptyBlocksByCityLevel(farm);
		_resetFarmNextBlocksByCityLevel(farm);
	};
	
	var _resetFarmEmptyBlocksByCityLevel = function(farm) {
		_cutNextBlocks(farm);
	...
	var _resetFarmNextBlocksByCityLevel = function(farm) {
		var totalcnt = m_this.getTotalBlockCount();
	...
	var _clearBlocksWhenChangeRole = function(netfarm, farm){
	...
	var _handleFarmData = function(netfarm) {
		if ( !netfarm ) return;
		_resetFarmBlocksByCityLevel(netfarm, m_curfarm);
		_clearBlocksWhenChangeRole(netfarm, m_curfarm);
		TQ.dictCopy(m_curfarm, netfarm);
		_resetSelCityToolFarmBtnState();
		ItemResUtil.initItemsres(m_curfarm.blocks);
		if ( m_this.isMyFarm(m_curfarm) ) {
			var myfarm = m_g.getImgr().getMyFarm();
			_resetFarmBlocksByCityLevel(netfarm, myfarm);
			TQ.dictCopy(myfarm, netfarm);
			ItemResUtil.initItemsres(myfarm.blocks);
			m_g.sendEvent({eid:EVT.FARMINFO,sid:2});
		}
		m_g.sendEvent({eid:EVT.FARMINFO,sid:0});
	};
	
	var _handleFarmInputResult = function(farminput) {
		if ( farminput && farminput.result != 0 ){
			m_g.sendEvent({eid:EVT.FARMINFO,sid:1, result:farminput.result});
		}
	};
	...
	var _resetSelCityToolFarmBtnState = function(){
...
FarmPresenter = Class.extern(function(){
	...
	this.isMyCurFarm = function(){
		return m_model.isMyCurFarm();
	};
	
	this.getCanGetMyRes = function(resid){
		return m_model.getCanGetMyRes(resid);
	};
	
	this.onGetResNums = function(nums){
		m_view.popGetResNum(nums);
	};
	...
	this.updateBlocksState = function(statechangeids) {
		var buildblocks = m_view.getCtrl('buildblocks');
	...
	var _regClickCaller = function(){
		m_view.getCtrl('myfarmbtn').setCaller({self:m_this, caller:_onClickMyFarm});
		m_view.getCtrl('farminfobtn').setCaller({self:m_this, caller:_onClickFarmInfo});
		m_view.getCtrl('farmrefreshbtn').setCaller({self:m_this, caller:_onClickFarmRefresh});
		m_view.getCtrl('farmrulebtn').setCaller({self:m_this, caller:_onClickFarmRule});
		
		m_view.getCtrl('opsel').setCaller({self:m_this, caller:_onClickOpSel});
		m_view.getCtrl('opinput').setCaller({self:m_this, caller:_onClickOpInput});
		m_view.getCtrl('opinit').setCaller({self:m_this, caller:_onClickOpInit});
		m_view.getCtrl('opget').setCaller({self:m_this, caller:_onClickOpGet});
		m_view.getCtrl('oppre').setCaller({self:m_this, caller:_onClickOpPreGet});
		m_view.getCtrl('opall').setCaller({self:m_this, caller:_onClickOpAllGet});
		m_view.getCtrl('opgetother').setCaller({self:m_this, caller:_onClickOpGet});
		m_view.getCtrl('opallother').setCaller({self:m_this, caller:_onClickOpAllGet});
		
		m_view.getCtrl('buildblocks').setCaller({self:m_this, caller:_onClickBlock});
		
		UIM.getDlg('selpip').setCaller({self:m_this, caller:_onSelPip});
	};
	
	var _regTipCaller = function(){
		m_view.getCtrl('buildblocks').setTipCaller({self:m_this, caller:_onGetBlockTip});
	};
	
	var _onFarmSvrPkg = function(netdata) {
		m_model.handleFarmSvrData(netdata);
	};
	...
	var _onCancelInput = function(){
		m_model.setOpState(FARMOP_STATE.SEL);
	};
	...
	var _onClickMyFarm = function(){
		_getFarmFromSvr(m_g.getImgr().getRoleId());
	};
	...
	var _onClickFarmRefresh = function(){
		var curFarm = m_model.getCurFarm();
		_getFarmFromSvr(curFarm.role.uid);
	};
	...
	var _onClickOpInit = function(){
		m_model.setOpState(FARMOP_STATE.INIT);
	};
	
	var _onClickOpGet = function(){
		m_model.setOpState(FARMOP_STATE.GET);
	};
	
	var _onClickOpPreGet = function(){
		m_model.setOpState(FARMOP_STATE.PREGET);
	};
	...
	var _onSelPip = function(resid, idx){
		m_model.setPipResid(resid);
		m_model.setOpState(FARMOP_STATE.INPUT);
	};
	
	var _onClickBlock = function(e, idx){
		var block = m_model.getBlock(_blockIdFromIdx(idx));
		if ( !block || (block.resid == FIXID.NEXTFARMBLOCK) ) return;
		if ( m_view.isDragged(e) ) return;
		
		var opstate = m_model.getOpState();
		if ( opstate == FARMOP_STATE.SEL ) {
			_handleSelStateClickBlock(block);
		}
		else if ( opstate == FARMOP_STATE.INPUT ) {
			_handleInputStateClickBlock(block);
		}
		else if ( opstate == FARMOP_STATE.INIT ) {
			_handleInitStateClickBlock(block);
		}
		else if ( opstate == FARMOP_STATE.PREGET ) {
			_handlePreGetStateClickBlock(block);
		}
		else if ( opstate == FARMOP_STATE.GET ) {
			_handleGetStateClickBlock(block);
		}
	};
	
	var _onGetBlockTip = function(data){
		var blockid = _blockIdFromIdx(data.idx);
		return m_model.getBlockTip(blockid);
	};
	
	var _onUpdate = function(cltTimeMs){
		m_view.getCtrl('buildblocks').updateTip();
		m_model.updateBlocks();
		m_view.refreshPopu();
	};
	
	var _handleSelStateClickBlock = function(block){
		if ( block.resid != FIXID.EMPTYFARMBLOCK ) return;
		UIM.getDlg('selpip').openDlg();
	};
	
	var _handleInputStateClickBlock = function(block){
		if ( block.resid != FIXID.EMPTYFARMBLOCK ) return;
		var pipresid = m_model.getPipResid();
		FarmSender.sendInputFarm(block.id, pipresid);
	};
	
	var _handleInitStateClickBlock = function(block){
		if ( block.resid == FIXID.EMPTYFARMBLOCK ) return;
		FarmSender.sendInitFarm(m_g.getImgr().getRoleId(), block.id);
	};
	
	var _handlePreGetStateClickBlock = function(block){
		if ( block.resid == FIXID.EMPTYFARMBLOCK ) return;
		if ( !m_model.isMyCurFarm() ) return;
		var res = m_model.getBlockRes(block);
		if ( res == 0 ) _sendSimulateGetResNumPkg(block.id);
		else FarmSender.sendPreGatherFarmRes(m_g.getImgr().getRoleId(), block.id);
	};
	
	var _handleGetStateClickBlock = function(block){
		if ( block.resid == FIXID.EMPTYFARMBLOCK ) return;
		if ( block.state != FARM_STATE.COMPLETE ) {
	...
	var _addCursors = function() {
		var dom = m_view.getCtrl('mousemap');
	...
	var _setSelOpBLinkBtn = function(opstate){
		if ( opstate == FARMOP_STATE.SEL ){
			//m_view.getCtrl('selopblink').stop();
		}
		else{
			//m_view.getCtrl('selopblink').start(-1);
		}
	};
	...
	var _blockIdFromIdx = function(idx) {
		return idx + 1;
	};
	
	var _getFarmFromSvr = function(roleId){
...
FarmView = CommMapPanel.extern(function(){
	//FarmView-testunit-start
	var C_BLOCK_W = 100;
	var C_BLOCK_H = 54;
	
	var C_MAINTOOLBAR_W = 442;
	var C_MAINBTNBAR_H = 50 - 18;
	var C_MAINTOOLBAR_H = 70;
	var C_SECTOOLBAR_W = 240;
	var C_SECTOOLBAR_H = 50;
	...
	this.open = function(){
		...
		m_g.getImgr().setCurLoadCity(FIXID.FARMMAP);
	...
	this.refreshPopu = function() {
		if (  m_model.isMyCurFarm() ) {
			TQ.setText(m_this.items.freepopu, m_g.getImgr().getIdlePopu());
		}
	};
	...
	this.getMainToolBarSize = function(){
		return {cx:C_MAINTOOLBAR_W, cy:C_MAINTOOLBAR_H};
	};
	
	this.getSecToolBarSize = function(){
		return {cx:C_SECTOOLBAR_W, cy:C_SECTOOLBAR_H};
	};
	
	this.setMainToolBarPos = function(pos){
		TQ.setDomPos(m_this.items.opbtnbar, pos.x, pos.y);
		TQ.setDomPos(m_this.items.roleinfobar, pos.x, pos.y+C_MAINBTNBAR_H);
	};
	...
	var _createOpToolBars = function(){
		...
		for ( var i=0; i<tmps.length; ++i ) {
			m_g.getGUI().buildDomItems(m_this.items.gamemap, tmps[i], null, m_this.items);
		}
	};
	...
	var _createBlocks = function(){
		...
		m_this.items.buildblocks = BuildBlocks.snew(m_g, 
			{
	...
	var _refreshTitle = function() {
		...
		var curphy = m_g.getImgr().getRoleAttrVal(ATTR.PS);
	...
	var _createNumberEffect = function(block, num){
		...
		var effect = m_g.getEntityfactory().allocNumEffect(m_this.items.mapscene);
		...
		effect.start();
		EUPD.appendEffect(effect, m_g.getEntityfactory().freeNumEffect);
	};	
