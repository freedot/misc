...
BuildingOpMgr = Class.extern(function(){
	//BuildingOpMgr-unittest-start
	var m_g=null;
	var m_this=null;
	var m_nullOpHdr=null;
	var m_opHdrs=[];

	this.init = function(g){
		m_g = g;
		m_this = this;
		m_nullOpHdr = BuildingInfoNullOpHdr.snew(m_g);
	...
	this.getStateName = function(item, groupIdx){
		...
		return _getHdrByBuildType(type).getStateName(item);
	};
	
	this.opSpeed = function(item, groupIdx){
		var type = m_g.getImgr().getTypeInAllBuildsGroup(groupIdx);
		_getHdrByBuildType(type).opSpeed(item);
	};
	
	this.opCancel = function(item, groupIdx){
		var type = m_g.getImgr().getTypeInAllBuildsGroup(groupIdx);
		_getHdrByBuildType(type).opCancel(item);
	};
	
	var _getHdrByBuildType = function(type){
		var hdr = m_opHdrs[type];
		if (!hdr) return m_nullOpHdr;
		return hdr;
	};	
...
BuildingInfoDlg = Class.extern(function(){
	//BuildingInfoDlg-unittest-start
	var m_g=null;
	var m_this=null;
	var m_dlg=null;
	var m_items={};
