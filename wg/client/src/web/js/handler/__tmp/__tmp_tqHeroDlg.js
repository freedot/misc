...
HeroArmTabView = Class.extern(function(){
	...
	_lc_.m_parent=null;
	_lc_.m_items=null;
	_lc_.m_presenter=null;
	_lc_.m_model=null;	
	_lc_.m_isArmPosItemsChanged = null;
	_lc_.m_wearListPos = [{x:38,y:8},{x:6,y:84},{x:71,y:84},{x:6,y:164},{x:71,y:164},{x:6,y:244},{x:71,y:244}];
	
	this.init = function(g, parent) {
		_lc_.m_this = this;
		_lc_.m_g = g;
		_lc_.m_parent = parent;
		_lc_.m_this.setItemChanged();
	};
	
	this.setPresenter = function(presenter) {
		_lc_.m_presenter = presenter;
	};
	...
	this.initDlg = function(items) {
		_lc_.m_items = items;
		_lc_._initWearListPos();
		_lc_._initArmsTab();
	};	
	
	this.update = function(){
		if ( !_lc_.m_parent.isShow() ) {
			return;
		}
		
		_lc_._updateWearList();
		_lc_._updateCurArmList();
	};
	...
	this.setItemChanged = function(){
		_lc_.m_isArmPosItemsChanged = [true, true, true, true, true, true, true, true];
	};
	
	_lc_._initWearListPos = function(){
		for ( var i=0; i<_lc_.m_items.wearList.getCount(); ++i ) {
			var item = _lc_.m_items.wearList.getItem(i).item;
			TQ.setCSS(item, 'float', 'none');
			TQ.setCSS(item, 'position', 'absolute');
			TQ.setDomPos(item, _lc_.m_wearListPos[i].x, _lc_.m_wearListPos[i].y);
		}
	};
	
	_lc_._initArmsTab = function(){
		_lc_._setArmsTabText();
		_lc_.m_items.armsTab.activeTab(1);
		_lc_.m_items.armsTab.hideTabCtrl();
	};
	
	_lc_._setArmsTabText = function(){
		for ( var i=0; i<_lc_.m_items.armsTab.getTabCount(); ++i ) {
			_lc_.m_items.armsTab.setTabText(i, rstr.herodlg.armTabs[i]);
		}
	};
	
	_lc_._updateWearList = function(){
		var hero = _lc_.m_model.getCurHero();
		if (!hero || !hero.wears) {
			_lc_._clearWearList();
			return;
		}
		
		_lc_._setWearList(hero);
	};
	
	_lc_._clearWearList = function() {
		for ( var i=0; i<_lc_.m_items.wearList.getCount(); ++i ) {
			var item = _lc_.m_items.wearList.getItem(i);
			_clearWearUIItem(item);
		}
	};
	
	_lc_._setWearList = function(hero) {
		for ( var i=0; i<_lc_.m_items.wearList.getCount(); ++i ) {
			var item = _lc_.m_items.wearList.getItem(i);
			var gridPos = i + 1;
	...
	_lc_._updateCurArmList = function(){
		var armPos = _lc_.m_items.armsTab.getActiveTab();
		if ( !_lc_.m_isArmPosItemsChanged[armPos] ) {
			return;
		} else {
			_lc_.m_isArmPosItemsChanged[armPos] = false;
		}
		
		var armList = _lc_.m_items.armsTab.getTabItems(armPos).armList;
		
		var armPosFilter = ArmPosFilter.snew(_lc_.m_g);
		var fitPosArms = armPosFilter.filter({armPos:armPos});
		
		var classRangeFilter = ItemClassRangeFilter.snew(_lc_.m_g);
		var arms = classRangeFilter.filter({classId:RES_CLS.HEROEQUIPITEM, items:fitPosArms});
		
		_lc_.m_model.setArmPosArms(armPos, arms);
		...
		_lc_.m_presenter.getArmPresenter().setArmListTipCaller();
	};
...
HeroDlgModel = Class.extern(function(){
	...
	this.handleHeroSvrData = function(ndata) {
		if ( ndata.data.heros ) {
			...
			_lc_._initHerosWearsItemres( heros.list );
			imgr.setHerosDefaultInfo();
			_lc_.m_g.pendReplaceEvent({eid:EVT.HERO_UPDATE, sid:0, pendtime:50});
		}
		
		if ( ndata.data.canusesstime != undefined ) {
			_lc_.m_g.getImgr().setCanUseSkillSteelTime( ndata.data.canusesstime );
			_lc_.m_g.pendReplaceEvent({eid:EVT.HERO_UPDATE, sid:0, pendtime:50});
		}
	};
	
	this.getOfficialRes = function(){
		var rolelevel = _lc_.m_g.getImgr().getRoleLevel();
	...
	this.getCurHero = function(){
		return _lc_.m_g.getImgr().getHeroByIdx(m_curheroidx);
	};
	...
	_lc_._initHerosWearsItemres = function(heros){
...
HeroArmTabPresenter = Class.extern(function(){
	...
	this.setCaller = function(){
		_lc_._setItems();
		_lc_._regItemChangeEvent();
		_lc_._setWearListCaller();
		_lc_._setArmsTabCaller();
		_lc_._setArmListCaller();
		_lc_._setWearListTipCaller();
		_lc_._setArmListTipCaller();
	};
	...
	_lc_._setItems = function(){
		_lc_.m_items = m_view.getCtrl('tablist').getTabItems(HERODLG_TAB_ARM_IDX);
	};
	
	_lc_._regItemChangeEvent = function(){
		_lc_.m_g.regEvent(EVT.PKG_CHANGE, 0, _lc_.m_this, _lc_._onItemChanged);
	};
	
	_lc_._setWearListCaller = function(){
		_lc_.m_items.wearList.setCaller({self:_lc_.m_this, caller:_lc_._onClickWearList});
	};
	
	_lc_._setArmsTabCaller = function(){
		_lc_.m_items.armsTab.setCaller({self:_lc_.m_this, caller:_lc_._onClickArmsTab});
	};
	
	_lc_._setArmListCaller = function(){
		for ( var i=0, n=_lc_.m_items.armsTab.getTabCount(); i<n; ++i) {
			var armList = _lc_.m_items.armsTab.getTabItems(i).armList;
			armList.setCaller({self:_lc_.m_this, caller:_lc_._onClickArmList});
		}
	};
	
	_lc_._setWearListTipCaller = function(){
		for ( var i=0, n=_lc_.m_items.wearList.getCount(); i<n; ++i ) {
			var item = _lc_.m_items.wearList.getItem(i);
			TTIP.setCallerData(item.exsubs.tooltips[TIP_PREFIX + 'item'], {self:_lc_.m_this, caller:_lc_._onGetWearArmTip}, {idx:i});
		}
	};
	
	_lc_._setArmListTipCaller = function(){
		for ( var tabIdx=0, tabCount=_lc_.m_items.armsTab.getTabCount(); tabIdx<tabCount; ++tabIdx ) {
			var armList = _lc_.m_items.armsTab.getTabItems(tabIdx).armList;
			for ( var listIdx=0, listCount=armList.getCount(); listIdx<listCount; ++listIdx ) {
				var item = armList.getItem(listIdx);
				TTIP.setCallerData(item.exsubs.tooltips[TIP_PREFIX + 'item'], {self:_lc_.m_this, caller:_lc_._onGetArmTip}, {idx:listIdx});
			}
		}
	};
	...
	_lc_._onClickWearList = function(e, idx){
		var hero = _lc_.m_model.getCurHero();
		if (!hero) {
			return;
		}
		var armPos = idx + 1;
		_lc_.m_items.armsTab.activeTab(armPos);
		
		var wear = hero.wears[armPos];
		if (wear) {
			HeroSender.sendUnWearArm(_lc_.m_g, hero.id, armPos);
		}
	};
	
	_lc_._onClickArmsTab = function(){
	...
	_lc_._onClickArmList = function(e, idx){
		...
		var armPos = _lc_.m_items.armsTab.getActiveTab();
		var arms = _lc_.m_model.getArmsByArmPos(armPos);
		var arm  = arms[idx];
		if (arm) {
			HeroSender.sendWearArm(_lc_.m_g, hero.id, arm.id);
		}
	};
	
	_lc_._onGetWearArmTip = function(data){
		var armPos = data.idx + 1;
		return _lc_._getWearArmTip(armPos);
	};
	
	_lc_._onGetArmTip = function(data){
		var arm = _lc_._getCurArm(data.idx);
		if ( !arm ) {
			return '';
		}
		
		var armTip = TIPM.getItemDesc(arm);
		var wearTip = _lc_._getWearArmTip(arm.itemres.apos);
	...
	_lc_._getWearArmTip = function(armPos){
		var hero = _lc_.m_model.getCurHero();
		if (!hero || !hero.wears) {
			return '';
		}
