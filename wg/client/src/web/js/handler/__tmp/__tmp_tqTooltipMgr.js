...
var _lc_={};this.lc=function(){return _lc_;};
	this.get = function(item, lbls, attrIds, color){
		...
		for ( var i=0; i<attrIds.length; ++i ) {
			...
			var attrVal = _lc_._getArmAttr(baseAttr);
			...
			attrVal += _lc_._getArmAttr(appendAttr);
			
			if ( hasBaseAttr && hasAppendAttr ) {
				attrVal += ')';
			}
			
			s += _lc_._getLabel(lbls, i, (attrId.base>0) ? attrId.base:attrId.append) + TQ.formatColorStr(attrVal, color) + C_TIP_NEWLINE;
		}
		return s;
	};
	
	_lc_._getArmAttr = function(attr){
		if (!attr) {
			return '';
		}
	...
	_lc_._getLabel = function(lbls, index, attrId){
		if (lbls != null) { 
			return lbls[index];
		}
...
SkillTooltip = Class.extern(function(){
	...
	this.getItemDesc = function(hero, skill){
		...
		s += _lc_._getSkillName(hero, skill);
		s += _lc_._getSkillFiveElem(hero, skill);
		s += _lc_._getSkillDescInfo(hero, skill);
		s += _lc_._getSkillDex(skill);
		s += _lc_._getNextSkillDesc(hero, skill);
		s += _lc_._getOpTip(skill);
		s += '</div>';
		return s;
	};
	
	_lc_._getSkillName = function(hero, skill){
	...
	_lc_._getSkillFiveElem = function(hero, skill){
		var s = rstr.skillTip.fiveElems[skill.itemres.fiveelem];
		return TQ.formatColorStr(s, COLORS.SKILL_TIP_FIVE_ELEM) + C_TIP_NEWLINE;
	};
	
	_lc_._getSkillDescInfo = function(hero, skill){
		var s = _lc_._getDescByLevel(skill.itemres, _lc_._getFinalSkillLevel(hero, skill));
		return TQ.formatColorStr(s, COLORS.SKILL_TIP_DESC) + C_TIP_NEWLINE;
	};
	
	_lc_._getSkillDex = function(skill){
	...
	_lc_._getNextSkillDesc = function(hero, skill){
		if ( skill.level == res_hero_baseskill_maxlevel ) return '';
		
		var s = C_TIP_NEWLINE;
		s += rstr.herodlg.lbl.nextskill + C_TIP_NEWLINE;
		s += _lc_._getDescByLevel(skill.itemres, _lc_._getFinalSkillLevel(hero, skill)+1);
		s += C_TIP_NEWLINE;
		s = TQ.formatColorStr(s, COLORS.SKILL_TIP_NEXTLEVEL);
		return s;
	};
	
	_lc_._getOpTip = function(skill){
		if ( skill.level == res_hero_baseskill_maxlevel ) return '';
		
		return C_TIP_NEWLINE + rstr.herodlg.lbl.steelskilltip;
	};
	
	_lc_._getDescByLevel = function(itemres, level) {
		var replaces = {};
	...
	_lc_._getFinalSkillLevel = function(hero, skill){
		var addLevelVal = AddLevelByHeroFiveElemAttr.getAddLevel(m_g, hero, skill.itemres);
...
BuildTooltip = function(){
	...
	this.getItemDesc = function(cityId, tag, item){
		m_cityId = cityId;
		return _lc_._getItemTipDesc(tag, item);
	};
	...
	_lc_._getItemTipDesc = function(tag, item){
		var cres = _getCommRes();
		...
		if ( tag == 'up' || tag == 'firstup' ){
			...
			return _lc_._getDownBuildTip(item, cres, tiptags);
		}
		else if ( tag == 'build' ){
	...
	var _getUpgradeBuildTip = function(item, cres, tiptags){
		...
		szTip += _lc_._replaceBuildDesc(item.level+1, levelres, item.itemres.desc);
		szTip += C_TIP_NEWLINE;
		szTip += C_TIP_NEWLINE;
	...
	_lc_._getDownBuildTip = function(item, cres, tiptags) {
		...
		if (preLevelRes){
			szTip += _lc_._replaceBuildDesc(item.level-1, preLevelRes, item.itemres.desc);
		} else {
	...
	var _getCommBuildTip = function(item, cres, tiptags){
		...
		stip += _lc_._replaceBuildDesc(item.level, levelres, item.itemres.desc);
		stip += '</div>';
		return stip;
	};
	
	_lc_._replaceBuildDesc = function(level, levelres, desc){
...
HeroTooltip = function(){
	...
	this.getNewHeroNatureFactorDesc = function(hero){
		return _lc_._getNewHeroNatureFactorDesc(hero);
	};
	...
	_lc_._getNewHeroNatureFactorDesc = function(hero){
		if ( !hero ) return '';
		var s = '<div class=itemtip>';
		s += NatureAttrsCommHdr(hero, hero.attrs, res_attrs);
		s += '</div>';
...
ItemTooltip = Class.extern(function(){
	var _lc_={};this.lc=function(){return _lc_;};
	_lc_.C_SMALLSPLITLINE = '<div style="HEIGHT:8px;FONT-SIZE:6px;OVERFLOW:hidden;"></div>';
	...
	this.getItemDesc = function(item, who, isWear){
		...
		s += _getItemIcon(item);
		s += _lc_._getItemName(item);
		//s += _lc_._getForceLevelStar(item);
		s += _lc_._getItemDescInfo(item);
		s += _lc_._getItemBindInfo(item);
		s += _lc_._getArmNeedLevel(item);
		s += _lc_._getArmBaseAttrs(item);
		s += _lc_._getArmSpeedAttr(item);
		s += _lc_._getArmSecondAttrs(item);
		s += _lc_._getArmSkillLevelAttrs(item);
		s += _lc_._getBesetGemsNumber(item);
		s += _lc_._getGemsList(item);
		s += _lc_._getSalePrice(item);
		s += _getBuyLimit(item);
		s += '</div>';
	...
	_lc_._getItemName = function(item){
		...
		s += '</b></center>';
		return s + _lc_.C_SMALLSPLITLINE;
	};
	
	_lc_._getForceLevelStar = function(item){
		if ( !item.flevel ) {
			return '';
		}
	...
	_lc_._getItemDescInfo = function(item){
	...
	_lc_._getItemBindInfo = function(item){
		if (_lc_.m_who == 'sys') return '';
	...
	_lc_._getArmNeedLevel = function(item){
	...
	_lc_._getArmBaseAttrs = function(item){
		var attrIds = [{base:ATTR.ST_B, append:ATTR.ST_A}
			,{base:ATTR.AG_B, append:ATTR.AG_A}
			,{base:ATTR.PH_B, append:ATTR.PH_A}
	...
	_lc_._getArmSpeedAttr = function(item){
	...
	_lc_._getArmSecondAttrs = function(item){
	...
	_lc_._getArmSkillLevelAttrs = function(item){
	...
	_lc_._getBesetGemsNumber = function(item){
	...
	_lc_._getGemsList = function(item){
	...
	_lc_._getSalePrice = function(item){
		if (_lc_.m_who == 'sys') return '';
		if (_lc_.m_who == 'other') return '';
