...
ExpedTargetChecker = Class.extern(function(){
	...
	this.init = function(g) {
		_lc_.m_g = g;
		_lc_.m_this = this;
		_lc_.m_target = ExpedTargetUtil.createNull();
	};
	
	this.setTarget = function(target){
		_lc_.m_target = ExpedTargetUtil.getValidTarget(target);
	};
	
	this.isSameAlliPlayer = function(){
		if ( !_lc_._isPlayerTarget() ) return false;
		if ( !_lc_.m_target.alliance ) return false;
		
		var myAlliance = _lc_.m_g.getImgr().getRoleRes().alliance;
		return (myAlliance.uid == _lc_.m_target.alliance.uid) && (myAlliance.uid != 0);
	};
	
	this.isSelfFieldTarget = function(){
		if ( !_lc_._isFieldTarget() ) return false;
		
		return _lc_.m_g.getImgr().getSelfFieldByGridId(_lc_.m_target.id) != null;
	};
	...
	this.isCopyFieldTarget = function(){
		return ( _lc_.m_target.type == OBJ_TYPE.COPYFIELD );
	};
	
	_lc_._isPlayerTarget = function(){
		return _lc_.m_target.type == OBJ_TYPE.ROLE;
	};
	
	_lc_._isFieldTarget = function(){
		return _lc_.m_target.type == OBJ_TYPE.FIELD;
	};
...
ExpedLineupInfoHdr = Class.extern(function(){
	...
	_lc_.m_this = null;
	_lc_.m_items = null;
	_lc_.m_myFightCapRefresher = null;
	_lc_.m_forceTabHdr = null;
	var m_targetHdr = null;
	
	this.init = function(g, uiitems){
		_lc_.m_this = this;
		_lc_.m_items = uiitems;
		_lc_.m_myFightCapRefresher = ExpedMyFightCapRefresher.snew(g, uiitems);
		_lc_.m_forceTabHdr = ExpedForceTabHdr.snew(g, uiitems);
		g.regEvent(EVT.HERO_UPDATE, 0, _lc_.m_this, _lc_._onUpdateHero);
	};
	...
	this.setTarget = function(target){
		_lc_.m_myFightCapRefresher.setTarget(target);
		_lc_.m_forceTabHdr.setTarget(target);
	};
	
	this.setExpedType = function(expedType){
		_lc_.m_myFightCapRefresher.setExpedType(expedType);
		_lc_.m_forceTabHdr.setExpedType(expedType);
	};
	
	this.setLineup = function(lineupId, heroIds){
		_lc_.m_myFightCapRefresher.setLineup(lineupId, heroIds);
		_lc_.m_forceTabHdr.setLineup(lineupId, heroIds);
		_lc_._setBtnsState();
	};
	
	this.refresh = function(){
		_lc_.m_myFightCapRefresher.refresh();
		_lc_.m_forceTabHdr.refresh();
		if ( m_targetHdr ) m_targetHdr.updateMoveNeedTime(_lc_.m_forceTabHdr.getArmySpeed() );
	};
	
	this.getLineup = function(){
		return _lc_.m_forceTabHdr.getLineup();
	};
	
	this.getHeroIds = function(){
		return _lc_.m_forceTabHdr.getHeroIds();
	};
	
	this.getFreeHeros = function(){
		return _lc_.m_forceTabHdr.getFreeHeros();
	};
	
	this.isNeedSingleHero = function(){
		return _lc_.m_forceTabHdr.isNeedSingleHero();
	};
	
	this.getCanExpedHeros = function(){
		return _lc_.m_forceTabHdr.getCanExpedHeros();
	};
	
	this.getExpedHerosInLineup = function(){
		return _lc_.m_forceTabHdr.getExpedHerosInLineup();
	};
	
	this._getArmySpeed = function(){
	};
	
	_lc_._setBtnsState = function(){
		if ( _lc_.m_myFightCapRefresher.getFirstValidHero() != null ) {
			_lc_.m_items.fillsoldier.enable(true);
			_lc_.m_items.treatment.enable(true);
		}
		else {
			_lc_.m_items.fillsoldier.enable(false);
			_lc_.m_items.treatment.enable(false);
		}
	};
	
	_lc_._onUpdateHero = function(){
		_lc_.m_this.refresh();
	};
...
ExpedMyFightCapRefresher = Class.extern(function(){
	...
	_lc_.m_items = null;
	_lc_.m_this = null;
	_lc_.m_targetChecker = null;
	_lc_.m_target = ExpedTargetUtil.createNull();
	_lc_.m_expedType = null;
	_lc_.m_heroIds = [];
	_lc_.m_lineupId = 0;
	...
	this.init = function(g, uiitems){
		...
		_lc_.m_this = this;
		_lc_.m_items = uiitems;
		_lc_.m_targetChecker = ExpedTargetChecker.snew(_lc_.m_g);
	};
	
	this.setTarget = function(target){
		_lc_.m_target = ExpedTargetUtil.getValidTarget(target);
		_lc_.m_targetChecker.setTarget(_lc_.m_target);
	};
	
	this.setExpedType = function(expedType){
		_lc_.m_expedType = expedType;
	};
	
	this.setLineup = function(lineupId, heroIds){
		_lc_.m_lineupId = lineupId;
		_lc_.m_heroIds = heroIds;
	};
	
	this.refresh = function(){
		TQ.setText(_lc_.m_items.myfightcap, 0);
		if ( _lc_.m_expedType == EXPED_TYPE.TIAOXIN 
			|| _lc_.m_expedType == EXPED_TYPE.DANTIAO ) {
			TQ.setText(_lc_.m_items.myfightcap, _lc_._getFirstHeroSingleFightCap());
		}
		else if ( _lc_.m_expedType == EXPED_TYPE.PAIQIAN ) {
			if ( _lc_.m_targetChecker.isSelfFieldTarget() ) {
				TQ.setText(_lc_.m_items.myfightcap, _lc_._getFirstHeroFightCap());
			}
			else {// target is alliance player
				TQ.setText(_lc_.m_items.myfightcap, _lc_._getHerosFightCap() );
			}
		}
		else if ( _lc_.m_expedType == EXPED_TYPE.TAOFA ) {
			TQ.setText(_lc_.m_items.myfightcap, _lc_._getHerosFightCap() );
		}
		else if ( _lc_.m_expedType == EXPED_TYPE.CUIHUI ) {
			TQ.setText(_lc_.m_items.myfightcap, _lc_._getHerosFightCap() );
		}
		else if ( _lc_.m_expedType == EXPED_TYPE.ZHANLING ) {
			TQ.setText(_lc_.m_items.myfightcap, _lc_._getFirstHeroFightCap());
		}
	};
	
	this.getFirstValidHero = function(){
		return _lc_._getFirstValidHero();
	};
	
	_lc_._getFirstHeroFightCap = function(){
		var hero = _lc_._getFirstValidHero();
		if ( !hero ) return 0;
		return _lc_.m_g.getImgr().getHeroAttrVal(hero, ATTR.FC);
	};
	
	_lc_._getFirstHeroSingleFightCap = function(){
		var hero = _lc_._getFirstValidHero();
		if ( !hero ) return 0;
		return _lc_.m_g.getImgr().getHeroAttrVal(hero, ATTR.SFC);
	};
	
	_lc_._getHerosFightCap = function(){
		if ( !_lc_._isValidLineUp() ) {
			return 0;
		}
		
		var fightCapSum = 0;
		for ( var i=0; i<_lc_.m_heroIds.length; ++i ) {
			var heroId = _lc_.m_heroIds[i];
			var hero = _lc_.m_g.getImgr().getHero( heroId );
			if ( !hero ) continue;
			
			fightCapSum += _lc_.m_g.getImgr().getHeroAttrVal(hero, ATTR.FC);
		}
		return fightCapSum;
	};
	
	_lc_._isValidLineUp = function(){
		var res = ItemResUtil.findItemres(_lc_.m_lineupId);
		return (res != null);
	};
	
	_lc_._getFirstValidHero = function(){
		for ( var i=0; i<_lc_.m_heroIds.length; ++i ) {
			var heroId = _lc_.m_heroIds[i];
			var hero = _lc_.m_g.getImgr().getHero( heroId );
			if ( hero ) return hero;
...
ExpedForceTabHdr = Class.extern(function(){
	...
	this.getLineup = function(){
		if ( _lc_.m_this.isNeedSingleHero() ) {
			return FIXID.DEFAULTLINEUP;
		}
		else {
			return _lc_.m_lineupId;
		}
	};
	
	this.getHeroIds = function(){
		return _lc_.m_heroIds;
	};
	
	this.getFreeHeros = function(){
		var heros = [];
		for ( var i=0; i<_lc_.m_heroIds.length; ++i ) {
			var hero = _lc_.m_g.getImgr().getHero( _lc_.m_heroIds[i] );
			if ( !hero ) continue;
	...
	this.refresh = function(){
		_lc_._updateLineupName();
		_lc_._updateForceTabList();
	};
	
	this.isNeedSingleHero = function(){
		if ( _lc_.m_expedType == EXPED_TYPE.TIAOXIN ) return true;
		if ( _lc_.m_expedType == EXPED_TYPE.DANTIAO ) return true;
		if ( _lc_.m_expedType == EXPED_TYPE.ZHANLING ) return true;
		if ( _lc_.m_targetChecker.isOtherFieldTarget() && _lc_.m_expedType == EXPED_TYPE.TAOFA ) return true;
		if ( _lc_.m_targetChecker.isSelfFieldTarget() && _lc_.m_expedType == EXPED_TYPE.PAIQIAN ) return true;
		
		return false;
	};
	
	this.getCanExpedHeros = function(){
		return _lc_._getCanExpedHeros();
	};
	
	this.getExpedHerosInLineup = function(){
		return _lc_._getExpedHerosInLineup();
	};
	...
	this.getArmySpeed = function(){
		...
		for ( var i=0; i<_lc_.m_heroIds.length; ++i ) {
			...
			var curSpeed = _lc_.m_g.getImgr().getHeroAttrVal(hero, ATTR.SP);
			if ( curSpeed < speed ) {
				speed = curSpeed;
			}
		}
	...
	_lc_._updateLineupName = function(){
		var res = ItemResUtil.findItemres(_lc_.m_lineupId);
		if ( res ) {
			TQ.setCSS(_lc_.m_items.curlineup, 'display', 'block');
			TQ.setText(_lc_.m_items.curlineup_txt, res.name);
		}
		else {
			TQ.setCSS(_lc_.m_items.curlineup, 'display', 'none');
		}
	};
	
	_lc_._updateForceTabList = function(){
		if ( _lc_.m_this.isNeedSingleHero() ) {
			if ( _lc_.m_heroIds.length > 1 ) {
				_lc_._showFirstHeroInLineupGrid();
			}
			else if ( _lc_.m_heroIds.length == 1 ) {
				_lc_._showHeroInMidGrid();
			}
			else {
				_lc_._showEmptyMidGrid();
			}
		}
		else {
			if ( _lc_.m_heroIds.length > 0 ) {
				_lc_._showHerosInLineupGrids();
			}
			else {
				_lc_._disableAllGrids();
			}
		}
	};
	
	_lc_._getCanExpedHeros = function(){
		var heroIds = _lc_._collectValidHeroIds();
		if ( _lc_.m_this.isNeedSingleHero() && heroIds.length > 0 ) {
			return [ heroIds[0] ];
		}
		
		if ( !_lc_.m_this.isNeedSingleHero() && (_lc_.m_heroIds.length == EXPED_TEAMGRID_MAXCNT) ) {
			return heroIds;
		}
	...
	_lc_._getExpedHerosInLineup = function(){
		var heroIds = _lc_._collectValidHeroIds();
		if ( _lc_.m_this.isNeedSingleHero() && heroIds.length > 0 ) {
			return [ heroIds[0] ];
		}
	...
	_lc_._collectValidHeroIds = function() {
		var heroIds = [];
		for ( var i=0; i<_lc_.m_heroIds.length; ++i ) {
			var heroId = _lc_.m_heroIds[i];
	...
	_lc_._showFirstHeroInLineupGrid = function(){
		_lc_._disableAllGrids();
		var heroGrid = _lc_._getFirstValidHeroGrid();
		if ( heroGrid ) {
			_lc_._enableGrid(heroGrid.grid);
			_lc_._showHeroInGrid(heroGrid.grid, heroGrid.hero);
		}
	};
	
	_lc_._showHeroInMidGrid = function(){
		_lc_._disableAllGrids();
		
		var midGridIdx = 4;
		_lc_._enableGrid(midGridIdx);
		
		var hero = _lc_.m_g.getImgr().getHero( _lc_.m_heroIds[0] );
		_lc_._showHeroInGrid(midGridIdx, hero);
	};
	
	_lc_._showEmptyMidGrid = function(){
		_lc_._disableAllGrids();
		
		var midGridIdx = 4;
		_lc_._enableEmptyGrid(midGridIdx);
	};
	
	_lc_._showHerosInLineupGrids = function(){
		_lc_._disableAllGrids();
		var res = ItemResUtil.findItemres(_lc_.m_lineupId);
		if ( !res ) {
			return;
		...
		for ( var i=0; i<res.grids.length; ++i ) {
			...
			var hero = _lc_.m_g.getImgr().getHero( heroId );
			if ( !hero ) {
				_lc_._enableEmptyGrid(gridIdx);
				continue;
			}
			
			_lc_._enableGrid(gridIdx);
			_lc_._showHeroInGrid(gridIdx, hero);
		}
	};
	
	_lc_._disableAllGrids = function(){
		for ( var i=0, cnt=_lc_.m_items.forcetablist.getCount(); i<cnt; ++i ) {
			_lc_._disableGrid(i);
		}
	};
	
	_lc_._disableGrid = function(idx){
		var listItem = _lc_.m_items.forcetablist.getItem(idx);
		_lc_._hideGridDetailLabels(idx);
		IMG.setBKImage(listItem.exsubs.icon.parentNode, IMG.makeImg('expedition/forcetab/disablebak.gif'));
	};
	
	_lc_._enableEmptyGrid = function(idx){
		var listItem = _lc_.m_items.forcetablist.getItem(idx);
		_lc_._hideGridDetailLabels(idx);
		IMG.setBKImage(listItem.exsubs.icon.parentNode, IMG.makeImg('expedition/forcetab/emptybak.gif'));
	};
	
	_lc_._enableGrid = function(idx){
		var listItem = _lc_.m_items.forcetablist.getItem(idx);
		_lc_._showGridDetailLbls(idx);
		IMG.setBKImage(listItem.exsubs.icon.parentNode, IMG.makeImg('expedition/forcetab/emptybak.gif'));
	};
	
	_lc_._hideGridDetailLabels = function(idx){
		_lc_._showOrHideGridDetailLabels(idx, 'hide');
	};
	
	_lc_._showGridDetailLbls = function(idx){
		_lc_._showOrHideGridDetailLabels(idx, 'show');
	};
	
	_lc_._showOrHideGridDetailLabels = function(idx, flag) {
	...
	_lc_._showHeroInGrid = function(gridIdx, hero){
		var imgr = _lc_.m_g.getImgr();
		var listItem = _lc_.m_items.forcetablist.getItem(gridIdx);
	...
	_lc_._getFirstValidHeroGrid = function(){
		var res = ItemResUtil.findItemres(_lc_.m_lineupId);
		if ( !res ) return null;
