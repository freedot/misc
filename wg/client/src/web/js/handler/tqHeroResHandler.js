...
RecruitHeroDlg = function(){
	...
	this.openDlg = function(){
		...
		_initInfo();
		m_g.regUpdater(m_this, _onUpdate, 1000);
		HelpGuider.getNewcomerSpirit().onDlgOpen('recruithero', {parent:m_dlg.getParent(), items:m_items});
	};
	...
	var _initDlg = function(){
		if ( !m_dlg ){
			...
			m_g.getGUI().initDlg(m_dlg, uicfg.hero.recruitherodlg, m_items);
			m_dlg.hide();
	...
	var _setBtnsCaller = function(){
		m_items.curherobtn.setCaller({self:m_this, caller:_onClickCurHero});
		m_items.refreshbtn.setCaller({self:m_this, caller:_onClickRefresh});
		m_items.buybtn.setCaller({self:m_this, caller:_onClickBuy});
	};
	...
	var _setListBtnsCaller = function(){
		for ( var i=0, n=m_items.list.getCount(); i<n; ++i ){
			...
			item.exsubs.recruitbtn.setCaller({self:m_this, caller:_onClickRecruit});
		}
	};
	...
	var _setListTooltip = function(){
		for ( var i=0, n=m_items.list.getCount(); i<n; ++i ){
			...
			TTIP.setCallerData(item.exsubs.tooltips['$nfitem'], {self:m_this, caller:_onGetNatureFactorTooltip}, {idx:i});
		}	
	};
	
	var _initInfo = function(){
		m_stoptime = parseInt(m_g.getSvrTimeMs()/1000) + 3600;
		_getNewHerosFromSvr();
	};
	
	var _updateInfo = function(newheros){
		_setHeroNumber();
		_setHeroList(newheros);
	};
	
	var _setHeroList = function(newheros){
		if ( !_isShow() ) return;
		for ( var i=0, n=m_items.list.getCount(); i<n; ++i ){
			var item = m_items.list.getItem(i);
			var hero = newheros[i];
	...
	var _setHeroNumber = function(){
		...
		TQ.setTextEx(m_items.curheronum, curnum + '/' + maxnum);
	};
	...
	var _onClickRefresh = function(){
		var _msgCallback = function(id){
			...
			if ( m_g.getImgr().getItemNumByResId(FIXID.REFRESHCARD) == 0 ){
				m_g.getGUI().msgBox(rstr.comm.msgts, rstr.recruitherodlg.norefresh, MB_F_CLOSE, null);
				return;
			}
	...
	var _hasMaxNValHero = function(){
		for ( var i=0; i<m_heros.length; ++i ) {
			if ( HeroNAttrFactorColorGetter.isMaxVal(m_heros[i]) ) return true;
		}
		return false;
	};
	...
	var _onClickRecruit = function(idx){
		...
		var maxnum = res_gethero_maxcnt( imgr.getRoleLevel() );
		if ( curnum >= maxnum ) {
			m_g.getGUI().sysMsgTips(SMT_WARNING, rstr.recruitherodlg.maxhero);
			return;
		}
		var _msgCallback = function(id){
			if ( id == MB_IDYES )  {
	...
	var _onSvrData = function(netevent){
		...
		if ( !newheros ) return;
		if ( newheros.del ) {
			TQ.dictCopy(m_heros, netevent.data.newheros.list);
		}
		else {
			m_heros = netevent.data.newheros.list;
		}
	...
	var _onUpdate = function(cltTimeMs){
		var curtime = m_g.getSvrTimeS();
		var lefttime = m_stoptime > curtime ? m_stoptime - curtime : 0;
		TQ.setTextEx(m_items.lefttime, TQ.formatTime(1, lefttime));
	...
	var _onGetTooltip = function(data){
		return TIPM.getNewHeroDesc(m_heros[data.idx]);
	};
	
	var _onGetNatureFactorTooltip = function(data){
		return TIPM.getNewHeroNatureFactorDesc(m_heros[data.idx]);
	};
	...
	var _refreshHerosFromSvr = function(){
