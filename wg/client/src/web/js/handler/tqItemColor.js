...
HeroNAttrColorGetter = Class.extern(function(){
	//HeroNAttrColorGetter-unittest-start
	var m_this = null;
	...
	this.getColorVal = function(prof, attr, val){
		var colorVal = TQ.formatColorStr(val, _getColor(prof, attr, val));
		return colorVal;
	};
	...
	var _getColor = function(prof, attr, val) {
...
HeroNAttrFactorColorGetter = Class.extern(function(){
	//HeroNAttrFactorColorGetter-unittest-start
	var m_this = null;
	var m_g = null;
	var m_valranges = [
		{min:3.3, max:12.1}
	...
	this.getNatureFactor = function(hero){
		return _getNatureFactor(hero);
	};
	
	this.getColorVal = function(hero){
		var val = _getNatureFactor(hero);
		var colorVal = TQ.formatColorStr(val, _getColor(val));
		return colorVal;
	};
	...
	this.getBorderClass = function(hero){
		var val = _getNatureFactor(hero);
		return 'item_icon_border_level' + _getLevel(val);
	};
	
	var _getColor = function(val) {
		var color = ITEM_COLORS[_getLevel(val)-1];
		if ( !color ) return ITEM_COLORS[0];
		return color;
	};
	
	var _getLevel = function(val){
	...
	var _getNatureFactor = function(hero){
		var nattrs = {};
