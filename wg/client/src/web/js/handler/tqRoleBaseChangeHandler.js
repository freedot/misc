...
RoleBasePanel = Class.extern(function(){
	...
	var _init = function(){
		...
		m_g.regEvent(EVT.NET, NETCMD.ROLEBASE, m_this, _onSvrPkg);
		m_g.regEvent(EVT.ROLEBASE, 2, m_this, _onRoleUpgrade); // level change
	};
	
	var _createPanel = function(){
		m_g.getGUI().initPanel(m_panel, uicfg.rolepanel, m_items);
		TQ.addEvent(m_items.icon, 'click', _onClickIcon);
		TQ.fixIE6Png(m_items.roleback);
		TQ.fixIE6Png(m_items.cityflag);
		TQ.fixIE6Png(m_items.level);
		_setTipsCallback();
	};
	...
	var _setTipsCallback = function(){
		...
		for ( var i=0; i<tipids.length; ++i ){
			var t = tipids[i];
			var tipid = m_items['tooltips'][TIP_PREFIX+t];
	...
	var _updateInfo = function(){
		...
		IMG.setBKImage(m_items.icon, IMG.makeMidImg(roleres.itemres.bigpic));
		TQ.setHtml(m_items.name, RStrUtil.makeXDiamondRoleName(roleres.name, roleres));
		TQ.setTextEx(m_items.level, roleres.level);
		TQ.setTextEx(m_items.alliance, roleres.alliance.name);
		m_items.exp.setRange(imgr.getRoleAttrVal(ATTR.NXP));
		m_items.exp.setValue(0,imgr.getRoleAttrVal(ATTR.XP));
