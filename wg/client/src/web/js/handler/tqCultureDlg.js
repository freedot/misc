...
CultureSvrPkgHdr = Class.extern(function(){
	//CultureSvrPkgHdr-unittest-start
	var m_g = null;
	var m_this = null;
	this.init = function(g) {
		_initParam(this, g);
	...
	var _regEvents = function(){
		...
		m_g.regEvent(EVT.NET, NETCMD.CULTURE, m_this, _onSvrPkg);
	};
	
	var _onLoginOk = function(){
		CultureSender.sendGetCultures(m_g);
	};
	
	var _onSvrPkg = function(netevent){
		var netdata = netevent.data;
		_onCulturesData(netdata);
		_onCultureLearningData(netdata);
	};
	
	var _onCulturesData = function(netdata){
		if ( !netdata.cultures ) return;
		
		var cultures = m_g.getImgr().getCultures();
		TQ.dictCopy(cultures, netdata.cultures);
		m_g.sendEvent({eid:EVT.CULTURE_UPDATE,sid:0});
	};
	
	var _onCultureLearningData = function(netdata){
