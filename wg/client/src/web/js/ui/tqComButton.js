...
ButtonBlinking = Class.extern(function(){
	//ButtonBlinking-unittest-start
	var C_DRTTIME = 250;
	var m_this = null;
	var m_timer = null;
	var m_interval = 0;
	var m_curtime = 0;
	var m_isBlinkingFrame = false;
	var m_resetStatusCaller = null;
	
	this.init = function(resetStatusCaller){
		m_this = this;
		m_resetStatusCaller = resetStatusCaller;
	};
	...
	this.start = function(interval){
		...
		m_interval = (interval > 0) ? interval : 0x7fffffff;
		m_curtime = 0;
	};
	
	this.stop = function(){
		_setBlinkingFrameFlag(false);
		_killTimer();
		_resetBtnStatus();
	};
	
	this.isBlinkingFrame = function(){
		return m_isBlinkingFrame;
	};
	
	var _onTimer = function(){
		m_isBlinkingFrame = !m_isBlinkingFrame;
	...
	var _setBlinkingFrameFlag = function(isBlinkingFrame){
		m_isBlinkingFrame = isBlinkingFrame;
	};
	
	var _killTimer = function(){
		if (m_timer) {
	...
	var _resetBtnStatus = function(){
		m_resetStatusCaller.caller.call(m_resetStatusCaller.self);
	};
