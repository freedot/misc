...
class MyShape(sae.RectangleShape):
	def __init__(self,canvas):
		...
		self.attrs =[
			...
			#-- dialog
			{'tag':'Modal',		'value':'', 'type':'string', 'get':None, 'set':None, 'IsReadOnly':None},
			{'tag':'Dlgpos',	'value':'', 'type':'string', 'get':None, 'set':None, 'IsReadOnly':None},
			{'tag':'Title',		'value':'', 'type':'string', 'get':None, 'set':None, 'IsReadOnly':None}
		]
		self.paths = None
	...
	def clone(self):
		...
		for attr in self.attrs:
			attrname = attr['tag']
			shape.SetAttrEx(attrname, self.GetAttr(attrname))
