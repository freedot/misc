require('tqBaseClass')
class LoadFontConfig(BaseClass)
	--初始化入口
	function Init(self)
	...
	--@param pathname
	-- xml资源路径
	function LoadXml(self, pathname)
	...
	function HandleItem(self, childres)
		if childres.name == 'attrs' then
			...
			self.fontManager:AddFontType(
				...
				retAttrs.BlockCount);
		elseif childres.name == 'itemref' then
			self.fontManager:AddRefFontType(childres.attr.name,childres.attr.refname)
		else
	...
	--@return
	-- 返回属性的一个table
	function HandleAttrs(self, attrsres)
