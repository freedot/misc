...
#ifdef LUA_OO
static void expr_field_or_objcall(LexState *ls, ExpDesc *v)
{
  FuncState *fs = ls->fs;
  ExpDesc key;
  expr_toanyreg(fs, v); /* ??? */
  lj_lex_next(ls);  /* Skip dot or colon. */
  expr_str(ls, &key);
  if ( ls->token == '(' || ls->token == TK_string || ls->token == '{' ) {
    bcemit_method(fs, v, &key); /* insert self in first call args*/
    parse_args(ls, v);
  } else {
    expr_index(fs, v, &key);
  }
...
static void expr_primary(LexState *ls, ExpDesc *v)
{
  ...
  for (;;) {  /* Parse multiple expression suffixes. */
    if (ls->token == '.') {
      ...
      expr_index(fs, v, &key);
#ifdef LUA_OO
    } else if (ls->token == TK_staticcall) {
      expr_field(ls, v);
#else // old lua
    } else if (ls->token == ':') {
      ExpDesc key;
